...
def register_volunteer(
    ...
    if len(db.collection('volunteers').where('email', '==', email).limit(1).get()) > 0:
        raise HTTPException(status_code=400, detail="Email already registered")
    if len(db.collection('volunteers').where('phone', '==', phone).limit(1).get()) > 0:
    ...
    db.collection('volunteers').document(vol_id).set(new_volunteer)
...
def login_volunteer(login_data: LoginRequest, db = Depends(get_db)):
    ...
    access_token = create_access_token(data={"id": str(volunteer.get('id')), "role": "volunteer"})
...
def add_certification(
    ...
    db.collection('certifications').document(cert_id).set(new_cert)
    return new_cert

@router.get("/dashboard")
def get_dashboard(db = Depends(get_db), current_volunteer: dict = Depends(get_current_volunteer)):
    certs_docs = db.collection('certifications').where('volunteerId', '==', current_volunteer.get('id')).get()
    certs = [doc.to_dict() for doc in certs_docs]
    return {
